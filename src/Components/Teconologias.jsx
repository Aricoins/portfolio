...
const DraggableIcon = styled.div`
  ...
  &:focus {
    ...
    transform: ${(props) =>
      props.isDragging ? "scale(1.1) rotate(5deg)" : "scale(1.1)"};
...
const DropZone = styled.div`
  ...
  border: 2px dashed
    ${(props) =>
      props.matched ? "#4ade80" : props.isOver ? "#60a5fa" : "#94a3b8"};
  border-radius: 8px;
  padding: 10px;
  text-align: center;
  color: ${(props) =>
    props.matched ? "#4ade80" : props.isOver ? "#60a5fa" : "white"};
  font-family: "Verdana";
  font-weight: bold;
  transition: all 0.3s ease;
  position: relative;
  ...
  &:focus {
    background: ${(props) =>
      props.matched ? "rgba(74, 222, 128, 0.3)" : "rgba(96, 165, 250, 0.2)"};
  ...
  return (
    <>
      <Tecnologi currentColor={currentColor} data-aos="fade-up">
        ...
        >
          Ofrecemos soluciones digitales innovadoras utilizando las últimas
          tecnologías del mercado. Nuestra experiencia y conocimiento abarcan
          una amplia gama de herramientas y frameworks, lo que nos permite
          ofrecer productos de alta calidad y rendimiento.
        </div>

        <GameContainer ref={gameContainerRef}>
          <GameTitle>🎮 Tech Match Challenge 🎮</GameTitle>
          <ScoreBoard>
            💰 {score} Aricoins | Objetivo: {totalPossibleCoins} | Aciertos:{" "}
            ...
            {draggedItem && (
              ...
              >
                <span style={{ color: "#fbbf24", fontWeight: "bold" }}>
                  {isDragging ? "🎯 Arrastrando: " : "✨ Seleccionado: "}
                  {draggedItem.name}
                </span>
                <button
            ...
            {technologies.map((tech) => (
              <DropZone
                ...
                onTouchEnd={(e) => {
                  ...
                  e.stopPropagation();
                  if (draggedItem) {
                    setDragOverZone(tech.id);
                    setTimeout(() => handleDrop(), 50);
                  }
                }}
              >
                {matches[tech.id]
                  ? "✅ "
                  : dragOverZone === tech.id
                  ? "🎯 "
                  : ""}
                {tech.name}
              </DropZone>
        ...
        >
          ...
          >
            <div>
              <strong style={{ color: "#fbbf24" }}>Frontend:</strong> React,
              Angular, Next.js, JavaScript, TypeScript, HTML, CSS, Redux,
              Zustand
            </div>
            <div>
              <strong style={{ color: "#4ade80" }}>Backend:</strong> Node.js,
              Express, Nest.js, GraphQL, Tanstack Query
            </div>
            <div>
              <strong style={{ color: "#f472b6" }}>Databases:</strong>{" "}
              PostgreSQL, MongoDB, Firebase
            </div>
            <div>
              <strong style={{ color: "#8b5cf6" }}>UI/UX:</strong> Tailwind CSS,
              Material UI, Bootstrap
            </div>
            <div>
              <strong style={{ color: "#06b6d4" }}>Tools:</strong> Git, Vercel,
              Swagger, Jest
            </div>
          </div>
        </div>
